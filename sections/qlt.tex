...
Introducing the resonant wavenumber \( k_{\rm res} \), defined as \( k_{\rm res} = \frac{\Omega}{v_\|} = \frac{v_\perp}{r_L v_\|} = \frac{1}{r_L} \frac{\sqrt{1 - \mu^2}}{\mu} = \frac{\tan \theta}{r_L} \), we can express $D_{\mu \mu}$ as follows\footnote{We use the property $\int dx \delta (c x) = \frac{1}{|c|} \int dx \delta (x)$}:
%
\begin{equation} 
