...
To calculate the gamma-ray intensity at \( E_\gamma = 1\)~GeV, we utilize a gas column density along the Galactic plane of \( N_{\rm H} \simeq 10^{22} \) cm\(^{-2}\), as inferred from radio observations~\TODO{add a ref.}:
. 
%
The proton intensity at \( E_p = 10\)~GeV, derived from direct measurements, is~\TODO{CRDB}:
%
\begin{equation}
...
This estimate closely aligns with the average value of the diffuse emission measured around the Galactic plane (within \( |b| < 5.0 \) degrees) for \( E_\gamma \simeq \) GeV, for instance, by the Fermi-LAT~\TODO{add a ref}.
